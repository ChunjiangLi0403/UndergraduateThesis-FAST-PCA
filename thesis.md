...
从时频信号图中，我们观察发现信号在1.15-1.2GHz波段存在着明显的强信号，根据其频段，我们能够确定这些强信号主要来自于空间人造卫星的电磁辐射。而从频率-频段图中，我们能够观察到一些比较明显的特征线，包括：频段在1.15-1.2GHz中的强卫星信号，频率在50Hz的宽带发射谱，频段在1.43GHz左右的中低频信号以及一系列低频的谱线，我们希望能够通过主成分分析将其中特征的线提取出来进行进一步的分析。

利用主成分分析方法将维度降到20维，下图展示了前几个主成分及他们对应的频段分布和成分占比：
...
可以发现其频段主要分布在1.15-1.2GHz之间，这与我们从频率-频段图中观察到的结果一致。然而，直接使用主成分分析对信号矩阵进行处理得到的前20维数据中，除了下图所示的第12个主成分之外，均分布在1.15-1.2GHz之间，占据原信号的比例过30%，且信号的波形均比较杂乱，阻碍了进一步的分析。
![01component12](https://user-images.githubusercontent.com/44969208/119221531-6e600200-bb22-11eb-8013-4edc5969ca95.png)
这个信号表现出了一定的谐波性质，我们将在后文具体讨论
...
相比于单个数据文件得到的结果，加大了信号的长度之后，主成分信号开始表现出时域特征，并且，我们再次观察到了与上文中提到的类似谐波的信号
![component16](https://user-images.githubusercontent.com/44969208/119221753-5ccb2a00-bb23-11eb-8007-2e5c5525e44d.png)

然而，在前20个主成分之中，依然有超过80%的频段分布集中在1.15-1.2GHz的卫星信号频段，这些信号或者已知，或者难以研究，并且FAST已经开始对人造卫星的射频干扰进行系统的研究。于是，我们对原数据进行频段上的滤波，仅保留频段在1.25-1.5GHz中间的信号，也就是仅保留后2048个数据维度，再进行主成分分析，得到的前几个主成分如下图所示：
...
通过比较可以发现此处的第四个主成分与上文提到的图Component16描述的是同一个信号的特征。
...
这些信号都表现出了一定的周期性，并且在频域上呈现集中分布的趋势，说明在1.25-1.30GHz以及1.45-1.50GHz频段中可能会发现新的特征干扰信号。

3. 在频域内进行主成分分析

由于我们发现了一系列带有周期性质的主成分信号，为了对窄带周期信号进行进一步的分析，我们尝试着在频域内进行主成分分析，选取10个数据文件先进行傅里叶变换到频域空间内，然后进行主成分分析，得到的前几个主成分为：
...
1. 各个频段中采集的频谱样本相差不大，频域内的变化可能没有时域内信号的变化更加显著，协方差矩阵的每个元素相差不大，难以选出更加有特征的成分谱 2. 每个频谱中不同频率的强度相差过大，且波动的规律性更弱 3. 频谱特征的分布距离高斯正态分布太远，这与主成分分析方法进行数据处理的出发点相悖，不太适用于主成分分析方法。
